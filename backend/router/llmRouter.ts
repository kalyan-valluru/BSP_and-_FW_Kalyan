...
export class LLMRouter {
  ...
  private determinePrimaryProvider(request: LLMRequest, config: RouterConfig): LLMProviderType {
    ...
    // Default: Gemini Cloud LLM is primary, Ollama is local fallback
    return config.enableCloud ? 'gemini' : 'ollama';
  }
  ...
  public async generate(request: LLMRequest): Promise<LLMResponse> {
    ...
    // Provider execution queue: Gemini primary -> Ollama local
    const providerQueue: LLMProviderType[] = ['gemini', 'ollama'];
