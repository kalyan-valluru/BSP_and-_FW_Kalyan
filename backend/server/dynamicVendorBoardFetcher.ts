...
export class DynamicVendorBoardFetcher {
  ...
  public async fetchBoardDocumentation(options: DynamicFetchOptions): Promise<DynamicFetchResult> {
    ...
    for (const query of searchQueries) {
      try {
        ...
        if (resp.ok) {
          ...
          const approvedPdfUrls = pdfUrls.filter((pdfUrl) => isAuthoritativeVendorUrl(pdfUrl, this.normalizeVendorKey(vendor)));
          for (const pdfUrl of approvedPdfUrls.slice(0, 4)) {
            console.log(`[DYNAMIC DOWNLOADING] Fetching official-vendor document -> ${pdfUrl}`);
            try {
    ...
    return {
      ...
      family,
      downloadedFiles: [],
      message: `No authoritative vendor document could be downloaded for '${options.processorName}'. No placeholder knowledge was created.`
    };
  }
  ...
  private normalizeVendorKey(vendor: string): string {
    if (vendor === 'st') return 'STMicroelectronics';
    if (vendor === 'xilinx') return 'AMD';
    if (vendor === 'nxp') return 'NXP';
    if (vendor === 'ti') return 'Texas Instruments';
    if (vendor === 'nvidia') return 'NVIDIA';
    if (vendor === 'raspberrypi') return 'Raspberry Pi';
    return Object.keys(OFFICIAL_VENDOR_REGISTRY).find(k => k.toLowerCase() === vendor.toLowerCase()) || vendor;
  }
