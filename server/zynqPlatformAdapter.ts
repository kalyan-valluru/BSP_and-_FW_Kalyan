...
`;
  ...
  async build(ctx: BuildContext): Promise<CompilationResult> {
    ...
    const requestedBoard = (ctx.metadata.boardName && ctx.metadata.boardName !== 'Unknown') ? ctx.metadata.boardName : 'ZedBoard Development Board';
    const resolvedPart = part;
    const resolvedBoard = isZynq7000Val ? 'xilinx.com:zedboard:part0:1.4' : 'xilinx.com:zcu102:part0:3.4';
...
catch { set_property board_part ${resolvedBoard} [current_project] }
