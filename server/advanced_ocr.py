...
# Normalization mapping for common OCR misreadings of engineering tokens
...
def extract_native_pdf(file_path: str, output_dir: str):
    ...
    try:
        ...
        for page_idx in range(page_count):
            ...
            blocks = []
            if text.strip():
                # Split text into non-empty lines as text blocks
            ...
            pages_data.append({
                ...
                "readingOrder": [i for i in range(len(blocks))]
            })

        # Evaluate if native text is sufficient (has text blocks and engineering labels)
        ...
        doc_json = {
            ...
            "engine": "PyPDFium2 Native Extractor",
            "pages": pages_data,
            "totalTextBlocks": total_text_blocks,
            "totalTables": 0,
            "engineeringLabelsCount": total_eng_labels,
...
def process_with_paddleocr(file_path: str, output_dir: str, prefer_vl: bool = False):
    """Process document or rendered page images using PaddleOCR (PPStructureV3 or PaddleOCRVL)."""
    try:
        import paddleocr
    ...
    if ext == '.pdf':
        if not pypdfium2_available:
            sys.stderr.write("[OCR] pypdfium2 required to render PDF pages for PaddleOCR\n")
            return None
        doc = pdfium.PdfDocument(file_path)
        page_count = len(doc)
        for i in range(page_count):
            ...
            image.save(img_path)
            temp_images.append((i+1, img_path, image.width, image.height))
    else:
        if pillow_available:
            img = Image.open(file_path)
            temp_images.append((1, file_path, img.width, img.height))
        else:
            temp_images.append((1, file_path, 1000, 1000))
        page_count = 1
    ...
    sys.stderr.write(f"[OCR] Initializing PaddleOCR Engine: {engine_name}...\n")
    try:
        if prefer_vl:
            try:
                engine = PaddleOCRVL()
            except Exception as e_vl:
                sys.stderr.write(f"[OCR] PaddleOCRVL initialization failed ({e_vl}), falling back to PaddleOCR...\n")
                engine = PaddleOCR(lang='en')
                engine_name = "PaddleOCR"
                strategy_name = "paddle_ocr_standard"
        else:
            engine = PaddleOCR(lang='en')
            engine_name = "PaddleOCR"
            strategy_name = "paddle_ocr_standard"
    ...
    for page_num, img_path, width, height in temp_images:
        blocks = []
        tables = []
        regions = []
        ...
        try:
            ...
            # Process PaddleOCR results structure
            if results:
                # If PaddleOCR returned a list containing dict or OCRResult
                res_list = results if isinstance(results, list) else [results]
                for item in res_list:
                    # Check for dict-like or object structure with rec_texts
                    if hasattr(item, 'rec_texts') or (isinstance(item, dict) and 'rec_texts' in item):
                        ...
                        # Standard PaddleOCR line output [[bbox], (text, conf)]
                        for line in item:
                            if isinstance(line, list) and len(line) >= 2 and isinstance(line[1], (tuple, list)):
                                ...
                                flat_box = [int(min(xs)), int(min(ys)), int(max(xs)), int(max(ys))]
                                blocks.append({
                                    "text": txt,
                                    "rawText": txt,
                                    "normalizedText": norm,
    ...
    # Cleanup temp images
    for _, img_path, _, _ in temp_images:
    ...
    avg_conf = (total_confidence_sum / total_confidence_count) if total_confidence_count > 0 else 0.0
...
def main():
    ...
    parser.add_argument("--inputPath", required=True, help="Path to document file")
    parser.add_argument("--outputDir", required=True, help="Path to output directory for JSON artifacts")
    parser.add_argument("--strategy", default="auto", help="OCR Strategy (auto, native, pp_structure_v3, paddleocr_vl)")
    ...
    if not os.path.exists(input_path):
        res = {"success": False, "error": f"Input file not found: {input_path}"}
        print(json.dumps(res))
        sys.exit(1)

    doc_json = None

    # Step 1: Native PDF check if auto or native requested
    if args.strategy in ["auto", "native"] and input_path.lower().endswith(".pdf"):
        sys.stderr.write("[OCR] Checking native PDF text extraction...\n")
        doc_json = extract_native_pdf(input_path, output_dir)

    # Step 2: PP-StructureV3 / PaddleOCR-VL if native not sufficient or requested
    if not doc_json:
        prefer_vl = (args.strategy == "paddleocr_vl")
        sys.stderr.write(f"[OCR] Invoking PaddleOCR Engine (prefer_vl={prefer_vl})...\n")
        doc_json = process_with_paddleocr(input_path, output_dir, prefer_vl=prefer_vl)

    # Step 3: Check success criteria
    if doc_json and doc_json.get("success"):
        # Save artifacts into output directory
        doc_ocr_path = os.path.join(output_dir, "document_ocr.json")
        with open(doc_ocr_path, "w", encoding="utf-8") as f:
            f.write(json.dumps(doc_json, indent=2))

        for page in doc_json.get("pages", []):
            page_num = page["pageNumber"]
            page_ocr_path = os.path.join(output_dir, f"page_{page_num:03d}_ocr.json")
        ...
        sys.stderr.write("[OCR] FAILED: No meaningful document content extracted.\n")
        fail_res = {
            "document": input_path,
            "pageCount": 0,
            ...
            "totalTextBlocks": 0,
            "totalTables": 0,
            "engineeringLabelsCount": 0,
            "averageConfidence": 0.0,
            "success": False,
            "error": "No meaningful document content extracted."
        }
